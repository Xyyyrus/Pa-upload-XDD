    ...
    <div className={`flex h-screen ${isDarkMode ? 'bg-gray-900' : 'bg-gray-100'}`}>
      ...
      <div className={`flex-1 ${isDarkMode ? 'text-white' : 'text-black'}`}>
        ...
        <section className='p-6 flex-1'>
          ...
          <div
            className={`p-4 shadow rounded-lg ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}
          >
            ...
            <table className='w-full mt-2'>
              ...
              <tbody>
                {admins.map((admin) => (
                  <tr key={admin._id} className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-300'}`}>
                    <td className={`py-2 ${isDarkMode ? 'text-white' : 'text-black'}`}>{admin.username}</td>
                    <td className={`py-2 ${isDarkMode ? 'text-white' : 'text-black'}`}>{admin.email}</td>
                    <td className={`py-2 ${isDarkMode ? 'text-white' : 'text-black'}`}>{admin.accountType}</td>
                    <td className={`py-2 ${isDarkMode ? 'text-white' : 'text-black'}`}>{admin.status}</td>
                    <td className='py-2'>
                      <button
                      ...
                      <button
                        className={`text-red-500 hover:underline ml-2 ${isDarkMode ? 'hover:text-red-400' : 'hover:text-red-600'}`}
                      >
                        Delete
                      </button>
                    </td>
